...
const Detail = ({ children }: DeatilProps) => {
  ...
  return (
    ...
    >
      {renderTarget && (
        <div className="border-grey200 size-full min-w-100 border-l p-5 pb-20">{renderTarget}</div>
      )}
    </motion.div>
  );
};
...
const List = ({ children }: React.PropsWithChildren<unknown>) => {
  return <div className="flex flex-[400_1] flex-col">{children}</div>;
};
...
const ListItem = ({ id, text, description, footer, header, onClick }: ListItemProps) => {
  ...
  return (
    <div
      className={clsx(
        'hover:bg-grey100 flex cursor-pointer items-center justify-between rounded-lg p-2',
