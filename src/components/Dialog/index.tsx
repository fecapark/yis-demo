...
const Content = ({ children, className }: React.PropsWithChildren<{ className?: string }>) => {
  return <div className={cn('flex max-w-180 flex-col px-6 pt-5 pb-5', className)}>{children}</div>;
};
...
const Button = ({
  ...
  return (
    <StyledButton className={cn('rounded-lg px-9 py-2', className)} size={size} {...props}>
      {children}
    </StyledButton>
  );
};
