...
export const TextInput = ({ className, description, invalid, label, ...props }: TextInputProps) => {
  const input = (
    <div className="flex flex-col gap-1.5">
      <input
        className={cn(
          'ease-ease w-full rounded-lg border px-4 py-2 transition-colors duration-200',
          invalid
            ? 'border-negative'
            : 'border-grey200 focus-visible:border-brandDisabled hover:border-brandDisabled',
          className,
        )}
        {...props}
      />
